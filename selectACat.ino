...
void loop() {
  int upperSensorVal = digitalRead(upperSensorPin);
  int lowerSensorVal = digitalRead(lowerSensorPin);
  int meowReceived = digitalRead(micPin);
  Serial.println(upperSensorVal);
  Serial.println(lowerSensorVal);
  Serial.println(meowReceived);
  
  if (upperSensorVal == LOW && lowerSensorVal == LOW && meowReceived == LOW) {
    ...
    //digitalWrite(servoPin, HIGH);
    //delay(10000); 
    while (upperSensorVal == HIGH && lowerSensorVal == LOW) {
      digitalWrite(servoPin, LOW);
    }
